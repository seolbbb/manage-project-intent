...
## 핵심 루프: 결정이 끝날 때까지 계속 질문합니다

이 Skill은 한 번 묻고 끝나는 요구사항 양식이 아닙니다. Full 작업에서는 **총 질문 수를 미리 제한하지 않습니다.** 한 라운드에 결과를 바꾸는 질문 1~3개를 묻고, 답변이 무엇을 바꾸는지와 그 근거를 기록한 뒤, 다음으로 남은 중요한 결정을 찾아 다시 묻습니다. 중요한 제품 결정을 추측하지 않고 구현할 수 있을 때에만 계획을 제시합니다.

종료 기준은 대화 피로도나 질문 수가 아니라 결정 범위입니다. 중요한 인터뷰 영역은 각각 다음 중 하나가 되어야 합니다.

- **answered:** 사용자 답변 또는 저장소 증거로 결정됨
- **deferred:** 나중에 결정하되, 담당자와 다시 열 조건이 기록됨
- **agent-owned:** 사용자가 해당 선택을 에이전트에게 명시적으로 위임함
- **not applicable:** 이 제품에 영향을 주지 않는 이유가 확인됨

이는 모든 기술 세부를 캐묻는 절차가 아닙니다. 제품 결과를 바꾸는 UX, 안전, 개인정보, 범위, 우선순위, 사업 조건, 허용할 수 없는 타협은 사용자가 결정합니다. 일반적인 구현 방식은 에이전트가 맡습니다. 예를 들어 사용자가 “배포 세부는 알아서 해”라고 하면, Skill은 일상적인 배포 질문을 멈추고 나중에 그 선택이 보호된 제품 결정과 충돌할 때만 이유를 설명하고 다시 묻습니다.
...
```mermaid
flowchart TD
    A["저장소 사실 조사"] --> B["중요한 질문 1~3개"]
    B --> C["답변·근거·바뀌는 계약 기록"]
    C --> D{"아직 결정되지 않은 중요한 선택이 있는가?"}
    D -- "예" --> B
    D -- "아니오: 결정 완료" --> E["계획 승인"]
    E --> F["프로젝트 기록 생성"]
    F --> G["한정된 작업 하나 구현"]
...
| 등급 | 적용 대상 | 동작 |
| --- | --- | --- |
| **Full** | 새 제품, 다중 마일스톤, 아키텍처, 안전·개인정보·데이터·라이선스·가격 결정 | 조사, 라운드당 1~3개 질문과 총량 제한 없는 결정 완료 인터뷰, 계획 승인, 관련 문서, 새 컨텍스트 인수인계 검증 |
...
- 여러 라운드의 Full 인터뷰, 명시적 위임 경계, Delta, Lite, status/audit, continue, revise, Spec Kit 경계와 인수인계 행동 사례
