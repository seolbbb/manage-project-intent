...
- User intent to preserve: 구현 전에 중요한 제품 결정을 질문으로 명시화하고, 목표·현재 상태·미래 순서·결정 근거를 분리해 보존하여 다음 에이전트가 대화 기록 없이도 같은 의도로 작업을 이어가게 한다.
...
- Job: 새 프로젝트는 충분히 질문한 뒤 시작하고, 기존 프로젝트는 실제 저장소와 문서에서 재개하며, 중요한 개입 이유를 미래 에이전트가 보존하게 한다.
...
- Full은 저장소 조사 후 한 번에 1~3개의 중요한 질문을 반복하고, 결정 완료 계획의 승인 뒤에만 문서와 코드를 변경한다.
...
- 새 프로젝트, Delta, Lite, status/audit, continue, revise, Spec Kit 양쪽, Goldfish 행동 평가 케이스가 모두 정의되고 대표 fresh-agent 실행이 통과한다.
